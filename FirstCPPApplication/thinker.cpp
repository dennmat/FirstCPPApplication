...
void Thinker::update(Game* game)
{
    ...
    if (game->player->IsActorInSight(this->master))
    {
        ...
        if (master->l_path == NULL)
        {
            ...
            // master->dest_x = rnd->getInt(1, 3);
            // master->dest_y = rnd->getInt(1, 3);
            master->dest_x = game->player->x;
            master->dest_y = game->player->y;
            // master->dest_x = 6;
            // master->dest_y = 3;
            cout << master->x << ", " <<  master->y << ", " <<  master->dest_x << ", " <<  master->dest_y << ", " << endl;
            int x, y, dest_x, dest_y;
            x = master->x;
            y = master->y;
            dest_x = master->dest_x;
            dest_y = master->dest_y;
            cout << x << ", " <<  y << ", " <<  dest_x << ", " <<  dest_y << ", " << endl;
            master->l_path->compute(x, y, dest_x, dest_y);
